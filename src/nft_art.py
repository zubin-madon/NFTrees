...
def nft_draw(nft, tree_seed=None):
    ...
    # ---GLOBALS------
    LEAVES = [(nft.token_id_list[i][0:6]) for i in range(len(nft.token_id_list))]  #['leaf' for i in range(1400)]
    ROOTS =  [(nft.from_address_list[i][0:4]) for i in range(len(nft.from_address_list))]  #['roots' for i in range(1400)]
